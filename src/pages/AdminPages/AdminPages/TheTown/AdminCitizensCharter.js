...
export default function AdminCitizenCharter() {
    ...
    const handleItemSelected = (item) => {
        for (var itemObject of items) {
            if (itemObject.charterName === item) {
                ...
                setCustomItemName(itemObject.charterName);
                switchMode('editing');
        ...
        axios.get(API.viewCharter, {})
            ...
            .then((data) => {
                ...
                for (var chart of data) {
                    newItemSidebarItems.push(chart.charterName)
                }
    ...
    const handleSaveAdd = () => {
        ...
        formData.append('charterName', customItemName);
        formData.append('pdf', selectedFile);
        formData.append('pdfName', selectedFile.name);

        axios.post(API.addCharter, formData, {
            headers: {
    ...
    const handleSaveEdit = () => {
        ...
        if (selectedItem.charterName !== customItemName) {
            hasChanges = true;
        }
        ...
        if (hasChanges) {
            ...
            const formData = new FormData();
            formData.append('charterID', selectedItem.charterID);
            formData.append('charterName', customItemName);
            formData.append('pdf', selectedFile);
            formData.append('pdfName', selectedFile.name);

            axios.post(API.editCharter, formData, {
                headers: {
                    'Authorization': `Bearer ${Cookies.get("token")}`,
                    'Content-Type': 'multipart/form-data', // Important for file uploads
                }
                ...
                .then((data) => {
                    ...
                    for (var item of items) {
                        newItems.push(item.charterID === selectedItem.charterID ? data : item);
                    }
                    setItems(newItems);

                    var newSidebarItems = [];
                    for (var item of itemSidebarItems) {
                        newSidebarItems.push(item === selectedItem.charterName ? customItemName : item);
                    }
    ...
    const handleEditItem = () => {
        if (customItemName.trim() !== '') {
            ...
            const updatedItems = itemSidebarItems.map((item) =>
                item === selectedItem.charterName ? customItemName : item
            );
    ...
    return (
        <>
                    ...
                    <div className='fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-80 h-32 rounded-10 bg-lgu-green z-70'>
                        ...
                        <div className='p-3 text-center'>
                            ...
                            <button
                                ...
                                onClick={(e) => {
                                    e.stopPropagation();
                                    var id = selectedItem.charterID;
                                    switchMode("");
                                    resetInputFields();
                                    axios.post(API.deleteCharter(id), {}, {
                                        headers: {
                                            'Authorization': `Bearer ${Cookies.get("token")}`,
                                        ...
                                        .then((response) => response.data)
                                        .then((data) => {
                                            setItems(prevItems => prevItems.filter(item => item.charterID !== id));
                                            setItemSidebarItems(prevItems => prevItems.filter(item => item !== selectedItem.charterName));
            ...
            <div className="flex flex-col">
                {/* Title */}
                <h1 className="text-4xl font-bold mb-2 mt-8 ml-2">Citizens' Charter</h1>
                ...
                <div className="flex">
                    ...
                    <div className="w-3/4 p-4">
                        <h2 className="text-2xl font-bold mb-2 mt-8">
                            {isAdding ? 'Add Custom Item' : isEditing ? 'Edit ' + selectedItem.charterName : 'Item'}
                        </h2>
