...
export default function AdminTransparency() {
    ...
    const handleItemSelected = (item) => {
        for (var itemObject of items) {
            if (itemObject.transparencyName === item) {
                ...
                setCustomItemName(itemObject.transparencyName);
                switchMode('editing');
        ...
        axios.get(API.viewTransparency, {})
            ...
            .then((data) => {
                ...
                for (var trans of data) {
                    newItemSidebarItems.push(trans.transparencyName)
                }
    ...
    const handleSaveAdd = () => {
        ...
        formData.append('transparencyName', customItemName);
        formData.append('pdf', selectedFile);
        formData.append('pdfName', selectedFile.name);

        axios.post(API.addTransparency, formData, {
            headers: {
    ...
    const handleSaveEdit = () => {
        ...
        if (selectedItem.transparencyName !== customItemName) {
            hasChanges = true;
        }
        ...
        if (hasChanges) {
            ...
            const formData = new FormData();
            formData.append('transparencyID', selectedItem.transparencyID);
            formData.append('transparencyName', customItemName);
            formData.append('pdf', selectedFile);
            formData.append('pdfName', selectedFile.name);

            axios.post(API.editTransparency, formData, {
                headers: {
                    'Authorization': `Bearer ${Cookies.get("token")}`,
                    'Content-Type': 'multipart/form-data', // Important for file uploads
                }
                ...
                .then((data) => {
                    ...
                    for (var item of items) {
                        newItems.push(item.transparencyID === selectedItem.transparencyID ? data : item);
                    }
                    setItems(newItems);

                    var newSidebarItems = [];
                    for (var item of itemSidebarItems) {
                        newSidebarItems.push(item === selectedItem.transparencyName ? customItemName : item);
                    }
    ...
    const handleEditItem = () => {
        if (customItemName.trim() !== '') {
            ...
            const updatedItems = itemSidebarItems.map((item) =>
                item === selectedItem.transparencyName ? customItemName : item
            );
                    ...
                    <div className='fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-80 h-32 rounded-10 bg-lgu-green z-70'>
                        ...
                        <div className='p-3 text-center'>
                            ...
                            <button
                                ...
                                onClick={(e) => {
                                    e.stopPropagation();
                                    var id = selectedItem.transparencyID;
                                    switchMode("");
                                    resetInputFields();
                                    axios.post(API.deleteTransparency(id), {}, {
                                        headers: {
                                            'Authorization': `Bearer ${Cookies.get("token")}`,
                                        ...
                                        .then((response) => response.data)
                                        .then((data) => {
                                            setItems(prevItems => prevItems.filter(item => item.transparencyID !== id));
                                            setItemSidebarItems(prevItems => prevItems.filter(item => item !== selectedItem.transparencyName));
                ...
                <div className="flex">
                    ...
                    <div className="w-3/4 p-4">
                        <h2 className="text-2xl font-bold mb-2 mt-8">
                            {isAdding ? 'Add Custom Item' : isEditing ? 'Edit ' + selectedItem.transparencyName : 'Item'}
                        </h2>
